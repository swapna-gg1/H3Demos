...
static unsigned int prgIntervalMs = 250;
...
static void APP_TouchDownHandler(const SYS_INP_TouchStateEvent* const evt)
{
    ...
    if((evt->y > 98 && evt->y < 198)&& (appData.scene == HOME))
    {
        panelTouch = true;
        lastX = evt->x;
        lastY = evt->y;
...
void Screen0_OnShow(void)
{
    ...
    Screen0_AdminPWDTextField->fn->setFont(Screen0_AdminPWDTextField,(leFont*)&regularFont11);
    ...
    StarStatic.fn->setFont(&StarStatic, (leFont*) &regularFont11);  //Set Font
    
    leFixedString_Constructor(&cookTimeStatic, cookTimeStaticBuff, 16); //Set data store
    cookTimeStatic.fn->setFont(&cookTimeStatic, (leFont*) &bold27);  //Set Font
        ...
        case SNAP_PANEL_SCREEN:
        {
            ...
            if (newPos < 0 && newPos > -930)
            {
                gfxcStartEffectMove(PANEL_CANVAS_ID, GFXC_FX_MOVE_DEC, 
                            x, 98, newPos, 98, LAYER_FX_MOVE_DELTA );
...
void event_Screen0_ShiftKey1Button_OnReleased(leButtonWidget* btn)
{
    ...
    {
        Screen0_keypadImage->fn->setImage(Screen0_keypadImage, (leImage*)&KeyboardCaps);
        capsOn = true;
    }
}
...
{
    Screen0_HomeGlowImage->fn->setPosition(Screen0_HomeGlowImage, 269, 414);
}
...
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&garlicKnotL);
    Screen0_CookingLabel->fn->setString(Screen0_CookingLabel, (leString*)&string_garlicKL);
    appData.screen_state = HOME_TO_COOKING_SCREEN;
}
void event_Screen0_ChickenButton_OnReleased(leButtonWidget* btn)
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&chickenL);
...
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&smokenSalmonL);
...
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&grilledSalmonL);
...
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&breakfastBurritoL);
...
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&croissantsL);
...
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&eggNtoastL);
...
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&frenchToastL);
    Screen0_CookingLabel->fn->setString(Screen0_CookingLabel, (leString*)&string_frenchToastL);
    appData.screen_state = HOME_TO_COOKING_SCREEN;
}
...
void event_Screen0_PancakeButton_OnReleased(leButtonWidget* btn)
{
    ...
    Screen0_CookingLabel->fn->setString(Screen0_CookingLabel, (leString*)&string_pancakeL);
    appData.screen_state = HOME_TO_COOKING_SCREEN;
     
}
void event_Screen0_QuesaButton_OnReleased(leButtonWidget* btn)
{
    Screen0_CookingImage->fn->setImage(Screen0_CookingImage, (leImage*)&quesadillaL);
...
void displaySettingsHome(void)
{
    ...
    if(appData.scene == COOKING_DETAIL || appData.scene == HOME)
    {
        ...
        //Screen0_HomeGlowImage->fn->setPosition(Screen0_HomeGlowImage, 527, 414);
...
void displayCookingScene(void)
{
    ...
    Screen0_BackBtnLabel->fn->setVisible(Screen0_BackBtnLabel, LE_FALSE);
    /* Move food image and label */
    Screen0_CookingImage->fn->setPosition(Screen0_CookingImage, 166, 130);
    Screen0_CookingLabel->fn->setPosition(Screen0_CookingLabel, 478, 110);
    ...
    Screen0_HomePanel->fn->setScheme(Screen0_HomePanel, &WhiteScheme);
    p=-10;
    cookTime1 = 12060;
    cookTime2 = 18000;
    cookTime3 = 5400;
...
void displayFoodComplete(void)
{
    ...
    Screen0_RoundGlowRingImage->fn->setImage(Screen0_RoundGlowRingImage, (leImage*)&roundGlowGreen);
...
void displayWifiSettings(void)
{
    gfxcShowCanvas(WIFISETUP_CANVAS_ID);
    gfxcCanvasUpdate(WIFISETUP_CANVAS_ID); 
...
void displayHomeScene(void)
{
    ...
    if(appData.scene == WIFI_SETUP)
    {
        ...
        gfxcHideCanvas(WIFISETUP_CANVAS_ID);
        gfxcCanvasUpdate(WIFISETUP_CANVAS_ID); 
        gfxcShowCanvas(HOME2_CANVAS_ID);
        gfxcCanvasUpdate(HOME2_CANVAS_ID);    
        gfxcShowCanvas(PANEL_CANVAS_ID);
        ...
        //Screen0_wifi1Button->fn->setEnabled(Screen0_wifi1Button, LE_FALSE);
        //Screen0_wifi2Button->fn->setEnabled(Screen0_wifi2Button, LE_FALSE);
        Screen0_SettingsButton->fn->setEnabled(Screen0_SettingsButton, LE_TRUE);
        Screen0_AdminPwdPanel->fn->setEnabled(Screen0_AdminPwdPanel, LE_FALSE);
...
void APP_Screen_Init ( void )
{
    ...
    gfxcSetWindowSize(SPLASH_BASE_LAYER, 800, 480);   
   
    gfxcSetLayer(SPLASH_SPRITE_LAYER, HW_LAYER1_ID);
    gfxcSetWindowPosition(SPLASH_SPRITE_LAYER, 235,209);
    gfxcSetWindowSize(SPLASH_SPRITE_LAYER, 348, 80);
    
    gfxcSetLayer(HOME1_CANVAS_ID, HW_LAYER1_ID);
    gfxcSetWindowPosition(HOME1_CANVAS_ID, 0,0);
    gfxcSetWindowSize(HOME1_CANVAS_ID, 800, 480);
            
    gfxcSetLayer(HOME2_CANVAS_ID, HW_LAYER2_ID);
    gfxcSetWindowPosition(HOME2_CANVAS_ID, 0,0);
    gfxcSetWindowSize(HOME2_CANVAS_ID, 800, 480);
    
    gfxcSetLayer(PANEL_CANVAS_ID, HW_LAYER3_ID);
    gfxcSetWindowPosition(PANEL_CANVAS_ID, 0,98);
    gfxcSetWindowSize(PANEL_CANVAS_ID, 1900, 100);
    
    gfxcSetLayer(COOKING_CANVAS_ID, HW_LAYER2_ID);
    gfxcSetWindowPosition(COOKING_CANVAS_ID, 0,0);
    gfxcSetWindowSize(COOKING_CANVAS_ID, 800, 480);
    
    gfxcSetLayer(SETTINGS_CANVAS_ID, HW_LAYER2_ID);
    gfxcSetWindowPosition(SETTINGS_CANVAS_ID, 0,0);
    gfxcSetWindowSize(SETTINGS_CANVAS_ID, 800, 480);

    gfxcSetLayer(SETTINGS2_CANVAS_ID, HW_LAYER3_ID);
    gfxcSetWindowPosition(SETTINGS2_CANVAS_ID, 0, 0);
    gfxcSetWindowSize(SETTINGS2_CANVAS_ID, 800, 480); 

    gfxcSetLayer(WIFISETUP_CANVAS_ID, HW_LAYER2_ID);
    gfxcSetWindowPosition(WIFISETUP_CANVAS_ID, 100,50);
    gfxcSetWindowSize(WIFISETUP_CANVAS_ID, 600, 370);
